...
def agent_native_prompt(
    ...
    return "\n".join(
        [
            ...
            "- If your runtime provides an automation note or memory surface, update it with concise aggregate run status only.",
            "- Include: Date and run window; Automation run status; Source/session counts processed; Memory/index change summary by count only; Review queue summary by count/status; Privacy/audit gate summary; Errors or blockers with actionable next step.",
            "- Do not include raw transcripts, raw prompts, environment blocks, command-progress narration, raw source refs, private case text, secret-like values, full queries, or unnecessary memory IDs.",
            "",
