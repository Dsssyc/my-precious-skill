...
`update-my-precious` 是 write-path skill。它扫描 source record 目录，以当前项目路径作为 source-record partition，写入选定 archive scope 加 source partition 下最后归档时间之后的新记录；如果同一 partition 中已归档过的同一个 source record 的 hash 发生变化，也会刷新该记录。默认 archive scope 是解析后的项目路径，以保持兼容；部署时可以用 `--archive-scope` 选择稳定的非项目记忆域。
...
project path 的稳定记忆域。增量 high-water 和 source hash freshness 仍按
archive scope 内的 source project partition 计算，因此同一 domain stream 中
某个项目的新记录不会吞掉另一个项目较旧但尚未归档的记录。
...
- `config/projects.jsonl`：全域 runner 使用的可选项目注册表；row 可包含
  `archive_scope`，让记忆域独立于 `project_path`；增量 high-water 仍在该
  domain 内按 source project partition 分开。
