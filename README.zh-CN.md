...
enabled project。
注册项目行可以包含 `archive_scope`，让 scheduled update 写入不等同于
project path 的稳定记忆域；也可以包含 `source_partition`，让 high-water 和
source hash freshness 跟随一个独立于 `project_path` 的稳定 source stream。
如果省略，source partition 默认是解析后的 project path，因此同一 domain
stream 中某个 project path 的新记录不会吞掉另一个 path 较旧但尚未归档的记录。
...
case ordinal、短 case-label hash、计数、noise-source bucket 和
`noise_relation_to_expected_at_5` 关系 bucket；后者只按相对 expected memory
的 layer/scope/topic 关系聚合计数，不渲染实际 layer/scope/topic 值。shadow
eval 不渲染 probe query、memory ID、source path、raw ref 或 forbidden
pattern。`--fail-under`、`--fail-over`、`--fail-under-file` 和
...
在生成答案之前，可以先审计私有或公开 generated-answer case set 是否具备可评分
条件，同时不渲染 case ID、query 或 reference answer：
...
这个 audit 只输出 aggregate JSON，统计总 case 数、positive/abstention 拆分、
reference-answer 覆盖、answer-scorable 覆盖、forbidden-pattern 覆盖、安全的
...
- `config/projects.jsonl`：全域 runner 使用的可选项目注册表；row 可包含
  `archive_scope`，让记忆域独立于 `project_path`；也可包含
  `source_partition`，让 high-water/source-hash stream 独立于 `project_path`。
...
- `index/source_partitions.jsonl`：每个 archive scope 加 source partition 一行。
...
- archive-scope 加 source-partition high-water marks，以及 source-record hash
  freshness 状态。
- 本地配置和日志。
- 配置好的 remotes。
